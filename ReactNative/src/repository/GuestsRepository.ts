...
export class GuestsRepository implements IBaseRepository<Guests> {
	async create(guest: Guests): Promise<Guests> {
		const response = await fetch("http://192.168.0.4:3333/guest", {
			method: "POST",
			headers: {
	...
	async update(guest: Guests, id: string): Promise<Guests> {
		const response = await fetch(`http://192.168.0.4:3333/guest/${id}`, {
			method: "PUT",
			headers: {
				"Content-Type": "application/json",
			},
	...
	async getAll(): Promise<Guests[]> {
		const response = await fetch("http://192.168.0.4:3333/guests");
		if (!response.ok) {
			throw new Error(`Erro na Requisição,status: ${response.status}`);
		}
